...
static double getCPUTimeAverage(const std::deque<std::chrono::microseconds>& cpu_time_history)
{
    if (cpu_time_history.empty()) return 0.0;
    auto sum = std::accumulate(cpu_time_history.begin(), cpu_time_history.end(), 0.0, [](double sum, const std::chrono::microseconds& duration) {
        return sum + duration.count();
    });
    return sum / cpu_time_history.size();
}
    ...
    return RPCHelpMan{
        ...
        RPCResult{
            ...
            {{
                ...
                {{
                    ...
                    {RPCResult::Type::NUM, "avgcputime", "Average CPU time consumption of the peer"},
                }},
            }},
        },
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            for (const CNodeStats& stats : vstats) {
                ...
                obj.pushKV("avgcputime", getCPUTimeAverage(stats.m_cpu_time_history));
